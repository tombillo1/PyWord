...
#issues: cant have words such as "taste" since setting 't' as grey messes with the words
#program does not keep track of where the yellow squares were set before

#max length of the words and format
MAX_LENGTH = 5
#list of invalid letters to be used in helper
invalid_letters = []
...
def wordle_helper(guess, word):
    ...
    filler_wrong = '&'
    filler_correct = '!'
    #Converts guess to list variable
    ...
    for words in sorted_data:
        ...
        for i in check_val:
            #checks for already correct letters
            if i.isalpha() and check_val[c] != check_words[c]:
               flag = True
            #checks for 'almost' correct letters
